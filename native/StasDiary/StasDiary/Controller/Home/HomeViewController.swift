...
class HomeViewController: UITableViewController {
    ...
    @objc func donePicker() {
        ...
        default:
            presenter.onIngredientListClick()
//            presenter.onAddIngredientClick()
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        switch viewType(from: indexPath) {
        case MealsViewModel.Companion.init().label_VIEW_TYPE:
            guard let vm = viewModel?.labelViewModelByPosition(position: position) else { return }
            presenter.onLabelClick(meals: vm.meals)
        case MealsViewModel.Companion.init().meal_ITEM_VIEW_TYPE:
            guard let vm = viewModel?.mealViewModelByPosition(position: position) else { return }
            presenter.onItemClick(item: vm.meal)
        default:
            print("Unkown type")
        }
    }
    ...
    private func viewType(from indexPath: IndexPath) -> Int32 {
        let position = Int32(indexPath.row)
        return viewModel?.viewTypeByPosition(position: position) ?? MealsViewModel.Companion.init().label_VIEW_TYPE
    }
}
